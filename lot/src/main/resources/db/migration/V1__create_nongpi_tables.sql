-- ============================================================
-- V1__create_nongpi_tables.sql  (Flyway 备份脚本)
-- 作用：提供可移植的 DDL 备份。首次部署使用 MCP mysql_execute 工具
--       创建数据库和表。该脚本在 Flyway 启用时也可作为迁移回退。
-- ============================================================

CREATE DATABASE IF NOT EXISTS nongpi CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE nongpi;
...
CREATE TABLE IF NOT EXISTS t_lot (
    ...
    status VARCHAR(16) NOT NULL DEFAULT 'IN_STOCK' COMMENT '批次状态',
    supplier_id BIGINT NOT NULL COMMENT '供应商ID',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (lot_no),
    INDEX idx_lot_fefo (temp_zone, status, expire_date) COMMENT 'FEFO先进先出索引'
...
CREATE TABLE IF NOT EXISTS t_alert_rule (
    id BIGINT AUTO_INCREMENT NOT NULL COMMENT '主键ID',
    sku_id BIGINT DEFAULT NULL COMMENT '商品SKU ID（NULL=全局规则）',
    temp_zone VARCHAR(16) DEFAULT NULL COMMENT '温区（NULL=所有温区）',
